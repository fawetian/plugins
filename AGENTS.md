...
- **Dual-platform default**: New skills must be available to both Claude and Codex unless the plugin is intentionally platform-specific (for example agents-only `devops` or Codex-only `goal-meta`)
