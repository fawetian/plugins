---
name: code-research
description: 深度研究和理解当前项目。当用户想要深入理解某个开源项目的设计思路、架构决策、Day 0 产品和技术方案意图、演进历史、实现地图、源码阅读路径、前置基础知识模块、基于源码证据的代码质量地图、核心机制、数据流或关键算法时，必须使用此 skill。触发词：code-research。
---
...
| 深入理解项目需要哪些基础知识 | learning_path + dependencies | 除非需要否则跳过 workflow |
...
- [templates/learning_path.md](templates/learning_path.md) — 基于目录结构和代码结构的源码阅读路径与前置基础知识模块
...
5. `05_workflow.md`
6. `06_learning_path.md` — 分析前置基础知识、项目目录和代码结构，并规划源码阅读路线
...
新增综合专题的依赖规则：
- `06_learning_path.md` 必须作为独立的源码阅读路径文件产出。它需要说明深入理解项目所需的前置基础知识模块；如果用户提供了自身背景（例如后端工程师、前端基础薄弱），要按这个背景调整补齐顺序；同时从阅读源码的角度解释目录结构、识别入口文件和核心模块、给出文件阅读顺序，并标出第一轮可以暂时跳过的目录。
...
完整研究的汇总必须包含：
- 来自 `06_learning_path.md` 的前置基础知识模块、源码阅读路线和目录结构重点
...
- **"我想贡献代码"** → 完整执行所有专题
- **"我想阅读源码"** → 学习路径专题 + 架构专题
- **"我是后端工程师，前端基础比较薄弱"** → 学习路径专题 + 依赖与生态专题
