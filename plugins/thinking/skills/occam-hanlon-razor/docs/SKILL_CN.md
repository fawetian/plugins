...
description: 奥卡姆剃刀与汉隆剃刀 — 优先选择最简单的解释，能用无知解释的不归因于恶意。当分析瘫痪、过度解读、或调试复杂系统时使用。触发词："occam"、"hanlon"、"奥卡姆"、"汉隆"、"剃刀"、"simplest explanation"、"最简单的解释"、"analysis paralysis"、"过度分析"。
userInvocable: true
---
...
## 核心理念
- **奥卡姆剃刀**：同等条件下，最简单的解释最可能是正确的
- **汉隆剃刀**：能用无知解释的，不要归因于恶意

## 约束
- 所有输出使用中文
- 必须先列出竞争解释再应用剃刀
- 剃刀是启发式工具，不是证明 — 标注可能误导的场景
- 使用 `templates/razor-analysis.md` 作为输出结构
